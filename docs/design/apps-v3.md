...
| Luviamo | intro video (5 MB, 16:9) and its poster; product landing screenshot (V4.5 cover) | `media/luviamo-intro.mp4` (copied from luviamo.app 19.9.2026, 5.1 MB, feedback item 7); `images/product-luviamo.webp` |
...
|---|---|---|---|---|
| A.1 | MasterForge plate | canvas sketch: gold meters and an EQ curve breathing | **The EQ capture on loop** (`captures/eq/eq.mp4`, 12.5 s, 3.4 MB), muted, cropped to 16:10, autoplays in view, poster frame from the clip, shown from the first paint (19.9.2026: the video was invisible until `canplay`, so a phone that refuses `play()` showed the sketch; now the poster shows at once and play is retried on the first touch or scroll); the sketch stays beneath as the last fallback. Caption `Pro Master · EQ, live` | OK (18.9.) |
...
| A.6 | Atlas plate | the SVG schematic (model in the middle, three doors, gates) | **Workspace board**: three columns (Projects, Waiting on, Decided) with cards that slide in, a partner avatar and a phone outline, a small library shelf below; the doors and gates go. Caption `Atlas · the workspace`. Card data 20.9.2026: `MasterForge · free tier rebuilt` and `Virustutka · autumn season` replace the two cards that carried a version (adr.sws.015) | OK (edit 20.9.) |
